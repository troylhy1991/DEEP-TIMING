...
class LabelImageQT1(QFrame):
	...
	def __init__(self, dataset_tag, nanowell_tag, init_t):
		...
		self.dataset = nanowell_tag.DATASET
		self.group = nanowell_tag.GROUP
		self.seq = nanowell_tag.SEQ
		self.global_seq = nanowell_tag.GLOBAL_SEQ
		self.E = nanowell_tag.E_num
		self.T = nanowell_tag.T_num
		self.t = init_t
		#self.t_span = self.dataset_tag.frames
		self.t_span = self.nanowell_tag.FRAMES
		self.TimInt = 5
	...
	def refresh_legacy(self):
		...
		#self.BID = nanowell_tag.BID
		#self.Well_ID = nanowell_tag.Nanowell_ID
		self.dataset = self.nanowell_tag.DATASET
		self.group = self.nanowell_tag.GROUP
		self.seq = self.nanowell_tag.SEQ
	...
	def getImagePath(self, CH):
		...
		path = self.path + str(self.dataset) + '/' + str(self.group) + '/' + str(self.seq) + '/imgs/' + 'CH' + str(CH) + '/' + str(self.t).zfill(5) + '.jpg'
		#print path
		return path
	...
	def getBoxPath(self, cell_type):
		if cell_type == 1:
			...
			# path = self.path + str(self.dataset) + '/' + str(self.group) + '/' + str(self.seq) + '/det/bbox_track/label_E_t' + str(self.t).zfill(3)+".txt"
			path = '/uhpc/roysam/hlu8/project/Cell-Tracking/Deep-TIMING/eval/figure3/GTS-600/labels/s' + str(self.global_seq).zfill(5) + '/label_E_t' + str(self.t).zfill(3)+".txt"
		...
		if cell_type == 2:
			...
			# path = self.path + str(self.dataset) + '/' + str(self.group) + '/' + str(self.seq) + '/det/bbox_track/label_T_t' + str(self.t).zfill(3)+".txt"
			path = '/uhpc/roysam/hlu8/project/Cell-Tracking/Deep-TIMING/eval/figure3/GTS-600/labels/s' + str(self.global_seq).zfill(5) + '/label_T_t' + str(self.t).zfill(3)+".txt"
			if os.path.isfile(path):
				return path
			else:
